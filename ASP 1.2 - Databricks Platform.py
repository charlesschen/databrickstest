...
# MAGIC %sql
# MAGIC SELECT traffic_source, device, SUM(ecommerce.purchase_revenue_in_usd) AS total_revenue
# MAGIC FROM events
# MAGIC GROUP BY traffic_source, device
...
# MAGIC %sql
# MAGIC SELECT ROUND(AVG(purchase_revenue_in_usd),2) AS average_purchase FROM sales 
...
# MAGIC %sql
# MAGIC SELECT event_name FROM events GROUP BY event_name ORDER BY event_name
